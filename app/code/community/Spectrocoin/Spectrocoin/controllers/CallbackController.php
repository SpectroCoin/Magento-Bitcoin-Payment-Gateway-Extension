...
class Spectrocoin_Spectrocoin_CallbackController extends Mage_Core_Controller_Front_Action
{
    ...
    public function _construct()
    {
        ...
        $publicKeyFilePath = Mage::getBaseDir('media') . '/spectrocoin/keys/' . Mage::getStoreConfig('payment/Spectrocoin/public_key_file');
        ...
        if (!file_exists($privateKeyFilePath) || !is_file($privateKeyFilePath)
            || !file_exists($publicKeyFilePath) || !is_file($publicKeyFilePath)
            || !$merchantId || !$appId) {
            exit;
        }
        $this->client = new SCMerchantClient($privateKeyFilePath, $publicKeyFilePath, $merchantId, $appId);
